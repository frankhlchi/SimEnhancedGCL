...
def negative_sample_idx_g(edge_index, labels, tau, f_sim_matrix, data_name, k=20,  alpha =0.1, beta = 0.5, cuda= True, verbose= True):
    n = len(labels)
    m = len(labels.unique())
    if verbose:
    ...
    if Path(ppr_filt_path).is_file():
        res = th.load(ppr_filt_path)
    else:
        labels_one_hot = F.one_hot(th.tensor([i for i in range(len(labels))]))
        LP = APPNP(k, alpha)
        res = LP(labels_one_hot.float(), edge_index = edge_index)
        th.save(res, ppr_filt_path)
    if cuda:
        res = res.cuda()
      
    feature_file_path = "./weight/feature_%s.pt"%(data_name)
    if Path(feature_file_path).is_file():
        res_ = th.load(feature_file_path)
    else:
        res_ = th.vstack([F.cosine_similarity(res[i], res, dim=0) for i in range(res.shape[0])])
        th.save(res_, feature_file_path)
...
def positive_sample_idx_g(edge_index, labels, tau, f_sim_matrix, data_name, k=20,  alpha =0.1, beta = 0.5, cuda= True, verbose= True):
    n = len(labels)
    m = len(labels.unique())
