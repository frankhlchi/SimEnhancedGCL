...
def train(neg_mask,pos_mask):
    ...
    loss = model.loss(z1, z2,neg_mask, pos_mask, batch_size=1024 if args.dataset == 'Coauthor-CS' or args.dataset == 'Coauthor-Phy' else None)
    loss.backward()
    optimizer.step()
...
def test(final=False):
    ...
    if args.dataset == 'WikiCS':
        ...
        for i in range(20):
            ...
            accs_vali.append(acc[0])
            accs_test.append(acc[1])
        acc = (sum(accs_vali) / len(accs_vali), sum(accs_test) / len(accs_test))
    else:
...
if __name__ == '__main__':
    ...
    tau_neg_prop = yaml.load(open('config.yaml'), Loader=SafeLoader)[args.dataset]['neg_tau']
    tau_pos_prop = yaml.load(open('config.yaml'), Loader=SafeLoader)[args.dataset]['pos_tau']
    beta = yaml.load(open('config.yaml'), Loader=SafeLoader)[args.dataset]['f_weight']
    print ('negative tau:', tau_neg_prop, ' positive tau:',tau_pos_prop,  ' beta:', beta)
    ...
    weight_dir = './weight'
    pos_mask = postive_sample_idx(data.edge_index, data.y.clone(), float(tau_pos_prop), feature = data.x.clone(), \
                                  k=20,  alpha =0.1, beta= beta, ppr_dir =weight_dir, data_name = args.dataset)
    neg_mask = negative_sample_idx(data.edge_index, data.y.clone(), float(tau_neg_prop),feature = data.x.clone(),\
                                   k = 20,  alpha = 0.1,beta= beta, ppr_dir = weight_dir, data_name = args.dataset)
    ...
    pd.DataFrame({'Time':[str(datetime.datetime.now())], 'seed':[args.seed], 'Valid_Accuracy':[acc[0]], 'Test_Accuracy':[acc[1]]}).to_csv(file_name, mode='a')
